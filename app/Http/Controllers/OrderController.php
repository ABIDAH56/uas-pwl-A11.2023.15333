...
class OrderController extends Controller
{
    ...
    public function create(Request $request) {
        ...
        $days = max(1, (int) $request->input('days', 1)); // minimal 1 hari
    
        // Hitung total harga: jumlah harga tiap item * hari sewa
        $totalHarga = 0;
        foreach($cart as $item) {
            // harga item sudah per durasi jam, kita kalikan hari sewa
            $totalHarga += $item->harga * $days;
        }
    
        $pembayaran = new Payment();
        $pembayaran->no_invoice = Auth::id()."/".Carbon::now()->timestamp;
        $pembayaran->user_id = Auth::id();
        $pembayaran->total = $totalHarga;
        $pembayaran->save();
        ...
        foreach($cart as $c) {
            Order::create([
                ...
                'durasi' => $c->durasi * $days, // durasi jam dikali hari
                'starts' => date('Y-m-d H:i', strtotime($request['start_date'].$request['start_time'])),
                'ends' => date('Y-m-d H:i', strtotime($request['start_date'].$request['start_time']."+".$c->durasi * $days." hours")),
                'harga' => $c->harga * $days,
            ]);
            $c->delete();
        }
